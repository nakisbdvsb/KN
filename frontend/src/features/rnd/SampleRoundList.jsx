          ...
          {kinds.map((tc) => {
            ...
            return (
              <div key={tc} data-testid={`sample-type-block-${p.supplier_id}-${tc}`}>
                <div className="flex flex-wrap items-center justify-between gap-2 border-b border-[#F4F5F7] bg-white px-3 py-1.5">
                  <p className="text-[11px] font-bold text-[#0058CC]">
                    ...
                    <span className="ml-2 font-normal text-[#6B6B73]">
                      {rounds.length} round
                      {rounds.length > 1 ? ` · hasil revisi ke-${rounds.length - 1}` : ""}
                ...
                {rounds.length > 0 && (
                  <div className="border-b border-[#F4F5F7] bg-[#FCFCFD] px-3 py-2">
                    <RoundBoxes testId={`sample-rounds-progress-${p.supplier_id}-${tc}`}
                      testPrefix={`sample-round-box-${p.supplier_id}-${tc}`}
                      items={rounds.map((r) => ({
                      ...
                      <div key={r.id} data-testid={`round-row-${r.id}`}
                        ...
                        <div className="flex flex-wrap items-center justify-between gap-2">
                          <p className="text-[11.5px] font-bold">
                            rnd {r.round_no}
                            <span className="ml-1.5 rounded bg-[#F5F5F7] px-1 py-px text-[9px] font-bold text-[#6B6B73]"
                              data-testid={`round-revision-${r.id}`}>{revisionLabel(r.round_no)}</span>
                            <span className="ml-2 font-normal text-[#6B6B73]">
                              tenggat {r.due_date || "—"}
