...
export default function RndSamplesView({ currentUser, selectedEntity, focus, onFocusConsumed }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((r) => {
      ...
      if (!term) return true;
      return [r.number, r.title, r.spec_number, r.color_target?.name,
      ...
      <div className="section-card">
        ...
        {loading ? (
          ...
          <div className="py-12 text-center text-[12px] text-[#6B6B73]"
            ...
            {minRevision ? (
              <p>Tidak ada sample dengan revisi ≥ {minRevision} — tidak ada yang tersendat di
                revisi berulang. Pilih <b>Semua ronde</b> untuk melihat seluruhnya.</p>
            ) : (
              ...
              return (
                <div key={s.id} data-testid={`rnd-sample-row-${s.id}`}
                  ...
                  <div className="min-w-0">
                    ...
                    <p className="flex flex-wrap items-center gap-1 text-[10px] text-[#6B6B73]">
                      <span>{(s.rounds || []).length} round · {formatCurrency(s.cost_total || 0)}</span>
                      <RevisionBadge count={sampleRevisionCount(s)} size="xs" testId={`rnd-sample-revision-${s.id}`} />
                      {overdue && <span className="font-bold text-[#C0392B]">· terlambat</span>}
                    </p>
                  </div>
