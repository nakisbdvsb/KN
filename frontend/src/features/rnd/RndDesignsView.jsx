...
import { DESIGN_STATUS_META, errMsg } from "./rndMeta";
...
const EMPTY = { q: "", status: "", cat: "", dcat: "", designer: "", tag: "", product: "", revision: "", score: "", artwork: "", line: "" };
...
export default function RndDesignsView({ currentUser, selectedEntity }) {
  ...
  const filtered = useMemo(() => {
    ...
    return rows.filter((d) => {
      ...
      if (!term) return true;
      return [d.code, d.title, d.story, d.category_name, d.design_category_name, d.created_by, ...(d.tags || [])].some((v) => (v || "").toLowerCase().includes(term));
    });
  }, [rows, f]);
    ...
    <div data-testid="rnd-designs-view">
      ...
      <div className="section-card mb-3">
        ...
        <div className="section-body space-y-2.5">
          <div className="grid grid-cols-2 gap-2 md:grid-cols-5" data-testid="rnd-designs-stats">
  ...
  return (
    <button type="button" onClick={onOpen} data-testid={`design-card-${d.id}`}
      ...
      <div className="relative flex h-28 items-center justify-center bg-[#F5F5F7]">
        ...
        <span className="absolute left-1.5 top-1.5"><RoundBadge design={d} testId={`design-round-${d.id}`} /></span>
        {["approved", "final_submitted", "active"].includes(d.status) && (
          <span className="absolute right-1.5 top-1.5 rounded bg-white/90 px-1.5 py-0.5 text-[9px] font-bold text-[#6B219A]">
            final {fin.colorway_files ?? 0}/{fin.colorway_required ?? 4} warna · {fin.mockup_files ?? 0} mockup
          </span>
        )}
      </div>
      <div className="space-y-1 p-2">
        <div className="flex items-center justify-between gap-1">
          ...
          <ScoreBadge value={d.current_score} acc={["approved", "final_submitted", "active"].includes(d.status)} testId={`design-score-${d.id}`} />
        </div>
