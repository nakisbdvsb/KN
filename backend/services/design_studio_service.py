...
FILE_KINDS = ("artwork", "reference", "mockup", "colorway", "ai_illustration")
...
def final_requirement(doc: Dict[str, Any]) -> Dict[str, Any]:
    """Berkas final wajib sesudah ACC: N varian warna (ditetapkan penilai) + minimal 1 mockup."""
    need_cw = int(doc.get("final_color_count") or DEFAULT_FINAL_COLOR_COUNT)
    cw = len(_files_of_kind(doc, "colorway"))
    mk = len(_files_of_kind(doc, "mockup"))
    return {"colorway_required": need_cw, "colorway_files": cw, "mockup_required": 1,
            "mockup_files": mk, "complete": cw >= need_cw and mk >= 1}
...
async def transition(gallery_id: str, action: str, actor: Dict[str, Any], note: str = "",
    ...
    if action == "submit_final":
        ...
        if not req["complete"]:
            raise DesignError(
                f"Berkas final belum lengkap: varian warna {req['colorway_files']}/{req['colorway_required']} "
                f"dan mockup {req['mockup_files']}/1. Mockup hasil desain yang di-ACC WAJIB diunggah.")
